<%@ WebHandler Language="C#" Class="types_ex" %>
...
public class types_ex : IHttpHandler {
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        filter.bitratemax = ClientUtils.FormatIntMax(context.Request["bitrate-max"]);
        filter.bitratemin = ClientUtils.FormatInt(context.Request["bitrate-min"]);
        filter.hmax = ClientUtils.FormatIntMax(context.Request["h-max"]);
        filter.hmin = ClientUtils.FormatInt(context.Request["h-min"]);
        filter.wmax = ClientUtils.FormatIntMax(context.Request["w-max"]);
        ...
        context.Response.Write(ServiceInvoke<ITags>.Invoke<string>(tag, proxy => proxy.VideoTypesEx(filter)));
    }
