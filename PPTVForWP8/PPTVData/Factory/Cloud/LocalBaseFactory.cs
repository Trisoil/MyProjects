...
namespace PPTVData.Factory.Cloud
{
        ...
        where T : new()
    {
        ...
        public void DeleteRecord(IList list)
        {
            while (list.Count > 0)
            {
                var info = list[0] as CloudDataInfo;
                if (info != null)
                {
                    info.DataType = CloudInfoEnum.Delete;
                    _postFactory.AddDeleteInfo(info);
                    base.DeleteRecord(info);
                }
            }
