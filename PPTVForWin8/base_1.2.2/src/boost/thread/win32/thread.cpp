...
// Windows CE doesn't define _beginthreadex
    ...
    bool thread::timed_join(boost::system_time const& wait_until)
    {
        ...
        if(local_thread_info)
        {
            if(!this_thread::interruptible_wait(local_thread_info->thread_handle,detail::get_milliseconds_until(wait_until)))
            {
                return false;
            }
            release_handle();
...
#if (!defined WINCE) && (!defined WINRT) && (!defined WIN_PHONE)
