...
#if defined(_MSC_VER) && !defined(_WIN32_WCE) && !defined(UNDER_CE) && !defined(WIN_PHONE)
...
{
    int cnt_t = cchWideChar;
    while(cchWideChar>0)
    {
        if(lpWideCharStr[cnt_t-cchWideChar]>0xFF){

        }else{
            lpMultiByteStr[cnt_t-cchWideChar] = lpWideCharStr[cnt_t-cchWideChar];
        }
        cchWideChar--;
    }
    return (cnt_t - cchWideChar);
}
...
conv_r = WCharToMultiByte(code_page, 0,  wide_lower_map, r,  this->m_lower_map, 1 << CHAR_BIT,  NULL, NULL);
