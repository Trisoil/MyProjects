...
#if (defined _MSC_VER) && (_MSC_VER >= 1200) && (!defined WIN_PHONE)
#  pragma once
...
static inline large_integer make_large_integer(__int64 i)
{
    return i;
}
...
extern "C" __declspec(dllimport) int __stdcall SetFileValidData(void *, large_integer size);
...
// VC ARM Compiler has some problems with reference to union
static inline large_integer make_large_integer(__int64 i)
{
	large_integer li;
	li.QuadPart = i;
...
{ return 0 != SetFilePointerEx((HANDLE)handle, make_large_integer(distance), (large_integer *)new_file_pointer, (DWORD)move_method);   }
