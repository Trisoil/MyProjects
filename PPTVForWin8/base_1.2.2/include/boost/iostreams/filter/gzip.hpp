...
>
struct cache_adapter
{
    typedef std::deque<char, Alloc> cache_t;
    ...
    { 
        size_t nc = cache_.size();
        if (cache_read_ + n > nc) {
            cache_.resize(cache_read_ + n);
            std::streamsize amt = iostreams::read(dev_, &*(cache_.begin() + nc), cache_read_ + n - nc);
            if (amt == -1) {
                cache_.resize(nc);
                return -1;
            }
        ...
        std::copy(cache_.begin() + cache_read_, cache_.begin() + cache_read_ + n, s);
        cache_read_ += n;
        return n;
    }
    ...
    void consume()
    {
        cache_.erase(cache_.begin(), cache_.begin() + cache_read_);
    }
...
public:
    ...
    std::streamsize read(Source& src, char_type* s, std::streamsize n)
    {
        if ((flags_ & f_header_read) == 0) {
            cache_adapter<Source, Alloc> nb(src, cache_);
            read_header(nb);
