...
public:
  ...
  private:
    ...
    friend class win_iocp_socket_service<Protocol>;
  ...
  {
#ifdef WINRT
    cancel_io_ex_ = CancelIoEx;
#else
    cancel_io_ex_ = (cancel_io_ex_t)::GetProcAddress(
      ...
      boost::system::error_code& ec)
  {
    ...
    else if (cancel_io_ex_)
    {
      socket_type sock = impl.socket_;
      HANDLE sock_as_handle = reinterpret_cast<HANDLE>(sock);
    //  if (!cancel_io_ex(sock_as_handle, 0))
      if (!cancel_io_ex_(sock_as_handle, 0))
      {
  ...
  {
  public:
    connect_operation(win_iocp_io_service& io_service, Handler handler)
      : operation(io_service,
          &connect_operation<Handler>::do_completion_impl,
          &connect_operation<Handler>::destroy_impl),
        io_service_(io_service),
        work_(io_service.get_io_service()),
        handler_(handler)
    {
  ...
  private:
    ...
    {
      // Take ownership of the operation object.
      typedef connect_operation<Handler> op_type;
      op_type* handler_op = static_cast<op_type*>(op);
      typedef handler_alloc_traits<Handler, op_type> alloc_traits;
      handler_ptr<alloc_traits> ptr(handler_op->handler_, handler_op);
  ...
  // Start an asynchronous connect. The peer_endpoint objects
  // must be valid until the connect's handler is invoked.
  template <typename Handler>
  void async_connect(implementation_type& impl,
      const endpoint_type& peer_endpoint, Handler handler)
  {
    ...
    raw_handler_ptr<alloc_traits> raw_ptr(handler);
    handler_ptr<alloc_traits> ptr(raw_ptr,
        iocp_service_, handler);

    // Accept a connection.
    DWORD bytes_read = 0;
    BOOL result = ::ConnectEx(impl.socket_,  peer_endpoint.data(), 
          peer_endpoint.size(), ptr.get());
    DWORD last_error = ::WSAGetLastError();
...
#ifdef WINRT
  typedef BOOL (* cancel_io_ex_t)(HANDLE, LPOVERLAPPED);
#else
  typedef BOOL (WINAPI* cancel_io_ex_t)(HANDLE, LPOVERLAPPED);
#endif
  cancel_io_ex_t cancel_io_ex_;
};
