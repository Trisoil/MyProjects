...
inline int gethostname(char* name, int namelen, boost::system::error_code& ec)
{
  ...
  || defined(BOOST_ASIO_DISABLE_ADDRINFO)
    ...
    boost::system::error_code& ec)
{
  clear_error(ec);
#if defined(BOOST_WINDOWS) || defined(__CYGWIN__)
# if defined(_WIN32_WINNT) && !defined(BOOST_ASIO_HAS_OLD_WIN_SDK) && (_WIN32_WINNT >= 0x0501) || defined(UNDER_CE)
...
# endif
  ...
  int error = getaddrinfo_emulation(host, service, hints, result);
  return ec = translate_addrinfo_error(error);
#elif defined(BOOST_ASIO_DISABLE_ADDRINFO)
  int error = getaddrinfo_emulation(host, service, hints, result);
  return ec = translate_addrinfo_error(error);
#else
  int error = ::getaddrinfo(host, service, hints, result);
