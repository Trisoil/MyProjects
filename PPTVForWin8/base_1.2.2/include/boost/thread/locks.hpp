...
#define BOOST_THREAD_NO_AUTO_DETECT_MUTEX_TYPES
    ...
    struct is_mutex_type
    {
        BOOST_STATIC_CONSTANT(bool, value = (boost::is_same<T, std::iterator_traits<T>::value_type>::value));
    };
#else
    template<typename T>
    struct is_mutex_type
    ...
    public:
#ifdef BOOST_NO_TEMPLATE_PARTIAL_SPECIALIZATION
        typedef unique_lock<Mutex> value_type;
#endif
        unique_lock():
            m(0),is_locked(false)
        {}
...
#else
        unique_lock(detail::thread_move_t1<upgrade_lock<Mutex> > other);
#endif
...
#else
        unique_lock& operator=(detail::thread_move_t1<upgrade_lock<Mutex> > other)
#endif
        {
            unique_lock temp(other);
            swap(temp);
    ...
    public:
#ifdef BOOST_NO_TEMPLATE_PARTIAL_SPECIALIZATION
        typedef shared_lock<Mutex> value_type;
#endif
        shared_lock():
            m(0),is_locked(false)
        {}
...
#else
        shared_lock(detail::thread_move_t1<unique_lock<Mutex> > other):
#endif
            m(other->m),is_locked(other->is_locked)
        {
            if(is_locked)
...
#else
        shared_lock(detail::thread_move_t2<upgrade_lock<Mutex> > other):
#endif
            m(other->m),is_locked(other->is_locked)
        {
            if(is_locked)
...
#else
        shared_lock& operator=(detail::thread_move_t1<unique_lock<Mutex> > other)
#endif
        {
            shared_lock temp(other);
            swap(temp);
...
#else
        shared_lock& operator=(detail::thread_move_t2<upgrade_lock<Mutex> > other)
#endif
        {
            shared_lock temp(other);
            swap(temp);
    ...
    public:
#ifdef BOOST_NO_TEMPLATE_PARTIAL_SPECIALIZATION
        typedef upgrade_lock<Mutex> value_type;
#endif
        upgrade_lock():
            m(0),is_locked(false)
        {}
...
#else
        upgrade_lock(detail::thread_move_t1<unique_lock<Mutex> > other):
#endif
            m(other->m),is_locked(other->is_locked)
        {
            if(is_locked)
...
#else
        upgrade_lock& operator=(detail::thread_move_t1<unique_lock<Mutex> > other)
#endif
        {
            upgrade_lock temp(other);
            swap(temp);
...
#else
    unique_lock<Mutex>::unique_lock(detail::thread_move_t1<upgrade_lock<Mutex> > other):
#endif
        m(other->m),is_locked(other->is_locked)
    {
        other->is_locked=false;
    ...
    namespace detail
    {
        ...
        public:
#ifdef BOOST_NO_TEMPLATE_PARTIAL_SPECIALIZATION
            typedef try_lock_wrapper<Mutex> value_type;
#endif
            try_lock_wrapper()
            {}
    ...
    namespace detail
    {
        ...
        {
            typedef typename boost::mpl::if_<is_mutex_type<Mutex>, int, Mutex>::type type;
        };
        /*
        template<typename Iterator>
