    ...
    namespace process
    {
        ...
        struct InternalMessage
        {
            struct Hook
                ...
                Hook,
                framework::memory::SharedMemoryPointerTraits<Hook, QUEUE_SHARED_MEMORY_INST_ID>
                >::type
            {
            };
        ...
        struct MemoryObject
            : framework::container::ListHook<
            MemoryObject
            , framework::memory::SharedMemoryPointerTraits<MemoryObject, QUEUE_SHARED_MEMORY_INST_ID>
            >::type
        {
        };
        ...
        public:
            ...
            void pop(MessageList & msg_list, Message const & hint, boost::uint32_t out_size)
            {
        ...
        private:
            User * add_and_get_user(std::string const & user)
            {
            ...
            User * get_user_by_id(boost::uint32_t id)
            {
                User * p_user = NULL;
                bool res = false;
                for(User::ListType::iterator iter = entry_->users.begin();
            ...
            User * find_user(std::string const & user)
            {
            ...
            User * find_user_from_cache(std::string const & user)
            {
                User * p_user = NULL;
                std::map<std::string, User *>::iterator iter;
                iter = user_cache_.find(user);
            ...
            void release_and_get_message_data(InternalMessage * imsg, std::string & data)
            {
                assert(imsg != NULL);
            ...
            void release_message_data(InternalMessage * imsg)
            {
                assert(imsg != NULL);
                MessageNode * node = imsg->next;
                MessageNode * delete_node = NULL;
            ...
            void alloc_message_space(char * buf, boost::uint32_t page_count)
            {
            ...
            bool push_message_to_user(Message const & msg, std::string const & user)
            {
                bool res = false;
                if (msg.level >= 8) {
            ...
            void insert_child_node(InternalMessage * imsg, std::string const & data)
            {
            ...
            boost::uint32_t message_childnode_size(InternalMessage const * imsg)
            {
                assert(imsg != NULL);
