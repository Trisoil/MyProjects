...
public:
  ...
  {
      per_descriptor_data() { shutdown_what_ = 0; }
      enum {
          shutdown_read = 1, 
          shutdown_write = 2, 
      };
  ...
  {
    descriptor_data.shutdown_what_ = 0;
    return 0;
  }
  ...
  void start_read_op(socket_type descriptor, per_descriptor_data & descriptor_data,
      Handler handler, bool /*allow_speculative_read*/ = true)
  {
    boost::asio::detail::mutex::scoped_lock lock(mutex_);
    if (!shutdown_) {
      if (descriptor_data.shutdown_what_ & per_descriptor_data::shutdown_read) {
        boost::system::error_code result(boost::asio::error::broken_pipe);
        std::size_t bytes_transferred(0);
        handler.perform(result, bytes_transferred);
        handler.complete(result, bytes_transferred);
      } else {
  ...
  template <typename Handler>
  void start_write_op(socket_type descriptor, per_descriptor_data & descriptor_data,
      Handler handler, bool /*allow_speculative_write*/ = true)
  {
    boost::asio::detail::mutex::scoped_lock lock(mutex_);
    if (!shutdown_) {
      if (descriptor_data.shutdown_what_ & per_descriptor_data::shutdown_write) {
        boost::system::error_code result(boost::asio::error::broken_pipe);
        std::size_t bytes_transferred(0);
        handler.perform(result, bytes_transferred);
        handler.complete(result, bytes_transferred);
  ...
  void shutdown_descriptor(socket_type descriptor, 
      per_descriptor_data & descriptor_data, socket_base::shutdown_type what)
  {
    boost::asio::detail::mutex::scoped_lock lock(mutex_);
    int my_what = 0;
    switch (what) {
      case socket_base::shutdown_both:
        my_what = per_descriptor_data::shutdown_read | per_descriptor_data::shutdown_write;
        break;
      case socket_base::shutdown_receive:
          my_what = per_descriptor_data::shutdown_read;
          break;
      case socket_base::shutdown_send:
        my_what = per_descriptor_data::shutdown_write;
        break;
    }
    descriptor_data.shutdown_what_ |= my_what;
    bool interrupt = false;
    if (descriptor_data.shutdown_what_ & per_descriptor_data::shutdown_read)
      interrupt = read_op_queue_.cancel_operations(descriptor) || interrupt;
    if (descriptor_data.shutdown_what_ & per_descriptor_data::shutdown_write)
  ...
  void close_descriptor(socket_type descriptor, per_descriptor_data & descriptor_data)
  {
    boost::asio::detail::mutex::scoped_lock lock(mutex_);
    cancel_ops_unlocked(descriptor);
    descriptor_data.shutdown_what_ = 0;
