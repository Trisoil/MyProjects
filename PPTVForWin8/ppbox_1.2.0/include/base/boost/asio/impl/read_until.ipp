    ...
    boost::system::error_code& ec)
{
  ...
  for (;;)
  {
    ...
    // for stupid vc6, we changed 'iterator' to 'iterator1' not to conflict with boost::iterator
    typedef boost::asio::buffers_iterator<const_buffers_type> iterator1;
    const_buffers_type buffers = b.data();
    iterator1 begin = iterator1::begin(buffers);
    iterator1 start = begin + next_search_start;
    iterator1 end = iterator1::end(buffers);

    // Look for a match.
    iterator1 iter = std::find(start, end, delim);
    if (iter != end)
    {
    ...
    boost::system::error_code& ec)
{
  ...
  for (;;)
  {
    ...
    // Look for a match.
    std::pair<iterator1, bool> result = boost::asio::detail::partial_search(
    ...
    boost::system::error_code& ec)
{
  ...
  for (;;)
  {
    ...
    // Look for a match.
    boost::match_results<iterator1> match_results;
    ...
    typename boost::enable_if<is_match_condition<MatchCondition> >::type*)
{
  ...
  for (;;)
  {
    ...
    // Look for a match.
    std::pair<iterator1, bool> result = match_condition(start, end);
    if (result.first != end)
    {
      if (result.second)
      {
...
namespace detail
{
  ...
  public:
        ...
        std::size_t bytes_transferred)
    {
      ...
      iterator1 begin = iterator1::begin(buffers);
      iterator1 start = begin + next_search_start_;
      iterator1 end = iterator1::end(buffers);

      // Look for a match.
      iterator1 iter = std::find(start, end, delim_);
      if (iter != end)
      {
        // Found a match. We're done.
    ...
    boost::asio::basic_streambuf<Allocator>& b, char delim, ReadHandler handler)
{
  ...
  // Look for a match.
  iterator1 iter = std::find(begin, end, delim);
  if (iter != end)
  {
    // Found a match. We're done.
    boost::system::error_code ec;
...
namespace detail
{
  ...
  public:
        ...
        std::size_t bytes_transferred)
    {
      ...
      // Look for a match.
      std::pair<iterator1, bool> result = match_condition_(start, end);
      if (result.first != end)
      {
        if (result.second)
        {
    ...
    typename boost::enable_if<is_match_condition<MatchCondition> >::type*)
{
  ...
  // Look for a match.
  std::size_t next_search_start;
  std::pair<iterator1, bool> result = match_condition(begin, end);
  if (result.first != end)
  {
    if (result.second)
    {
