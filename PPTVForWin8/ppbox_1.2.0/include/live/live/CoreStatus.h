...
struct CCoreStatus
{
    ...
    boost::uint64_t	m_TotalUploadBytes;		// 上传的数据总量(字节数)
	boost::uint64_t	m_TotalDownloadBytes;	
...
struct CCoreParameter
{
	...
	//unsigned short	m_usTCPUPNPPort;	//TCP在UPNP映射的外网端口，客户端填写，非0表示UPNP成功;0表示UPNP没有映射或者映射失败
  	...
  	boost::uint32_t		m_dwFileVersionMS;		//内核版本信息，内核填写，The most significant 32 bits of the file's binary version number.（见MSDN：VS_FIXEDFILEINFO结构体）
	boost::uint32_t		m_dwFileVersionLS;		//内核版本信息，内核填写，The least significant 32 bits of the file's binary version number.（见MSDN：VS_FIXEDFILEINFO结构体）
  	//live
