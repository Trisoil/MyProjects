...
LIVE_DECL bool LiveStartup(int node_type = 0);
...
struct LIVE_CHANNEL_STATUS
{
	...
	boost::uint64_t	TotalUploadBytes;		// 上传的数据总量(字节数)
	boost::uint64_t	TotalDownloadBytes;		// 下载的数据总量(字节数)
...
struct PPLIVE_COMMON_CHANNEL_STATUS
{
	...
	boost::uint64_t	m_TotalDownloadBytes;		// 下载的数据总量(字节数)
