...
public:
        ...
        char const * playlink)
    {
        ...
        if (async_) {
            PPBOX_DispatchAsyncOpen(playlink, static_call_back);
        } else {
            boost::thread th(boost::bind(&DispatchOpenManager::open_thread, playlink));
        }
...
private:
    static void open_thread(
        char const * playlink)
    {
        PP_int32 ec = PPBOX_DispatchOpen(playlink);
...
private:
    static DispatchOpenManager * inst_;
...
DispatchOpenManager * DispatchOpenManager::inst_ = NULL;
