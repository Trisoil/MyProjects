...
#ifndef _LIVE_P2PCOMMON2_BASE_PPL_NET_IP_H_
#define _LIVE_P2PCOMMON2_BASE_PPL_NET_IP_H_
...
inline void LoadLocalIPs(std::vector<u_long>& ipArray)
{
            ...
            if (!ioctl (fd, SIOCGIFCONF, (char *)&ifc)) {
                ...
                while (intrface-- > 0) {
                    if (!(ioctl (fd, SIOCGIFADDR, (char *) &buf[intrface]))) {
                        if (string(buf[intrface].ifr_name) == "lo")
                            continue;
                        if (buf[intrface].ifr_addr.sa_family == AF_INET) {
                            const sockaddr_in & sa = (sockaddr_in const &)buf[intrface].ifr_addr;
                            ipArray[ipIndex++] = sa.sin_addr.s_addr;
                        }
                    }
                } // while
