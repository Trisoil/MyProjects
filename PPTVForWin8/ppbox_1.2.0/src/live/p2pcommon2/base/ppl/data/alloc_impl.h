...
#ifndef _LIVE_P2PCOMMON2_BASE_PPL_DATA_ALLOC_IMPL_H_
#define _LIVE_P2PCOMMON2_BASE_PPL_DATA_ALLOC_IMPL_H_
...
public:
	...
	void init(size_t itemSize, size_t itemCount = 10)
	{
		...
		LIVE_ASSERT(m_itemCount > 0);
		LIVE_ASSERT(m_itemSize > 0);
//		LIVE_ASSERT(m_itemSize == 32 || m_itemSize == 64 || m_itemSize == 128 || m_itemSize == 256 || m_itemSize == 512 || m_itemSize == 1024 || m_itemSize == 2048);
	}
...
//		AutoLightMutex lock(m_lock);
		...
		{
#if !defined(_WIN32_WCE)
			LIVE_ASSERT(_CrtIsValidHeapPointer(&*m_blocks.begin()));
#endif
		}
		LIVE_ASSERT(m_itemSize > 0);
		if (m_free.empty())
		...
		LIVE_ASSERT(!m_free.empty());
		slist_node_base* itemNode = m_free.pop();
		LIVE_ASSERT(itemNode != 0);
...
//		AutoLightMutex lock(m_lock);
	...
	void check() const
	{
		...
		LIVE_ASSERT(m_totalItems >= 0);
		//LIVE_ASSERT(m_totalItems >= m_free.size());
	}
...
{
	LIVE_ASSERT(memchr(p, MEM_UNINITIALIZED, size) == 0);
}
