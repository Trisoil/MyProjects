...
#ifndef _LIVE_P2PCOMMON2_BASE_PPL_DATA_STLUTILS_H_
#define _LIVE_P2PCOMMON2_BASE_PPL_DATA_STLUTILS_H_
...
public:
	...
	static ValueT get_value(const ContainerT& coll, const ContainerT::key_type& key, const ValueT& defaultVal )
	{
...
#else
	template <typename ContainerT>
	static typename ContainerT::mapped_type get_value(const ContainerT& coll, const typename ContainerT::key_type& key, const typename ContainerT::mapped_type& defaultVal = ContainerT::mapped_type() )
	{
		typename ContainerT::const_iterator iter = coll.find(key);
		if (iter != coll.end())
			return iter->second;
