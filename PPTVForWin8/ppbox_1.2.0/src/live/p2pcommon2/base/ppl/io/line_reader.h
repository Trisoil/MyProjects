...
#ifndef _LIVE_P2PCOMMON2_BASE_PPL_IO_LINE_READER_H_
#define _LIVE_P2PCOMMON2_BASE_PPL_IO_LINE_READER_H_
...
public:
	static std::string read_line()
	{
		//static HistoryInitializer historyInit;
/*		char* sz = ::readline(NULL);
		if ( NULL == sz )
			return std::string();
		std::string line(sz);
		free(sz);
		return line;*/
		static swift::SReadline myReadLine;
		static std::string prompt;
		bool endOfInput;
		return myReadLine.GetLine(prompt, endOfInput);
	}
};
