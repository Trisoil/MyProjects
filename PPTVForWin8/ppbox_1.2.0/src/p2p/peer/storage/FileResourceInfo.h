...
namespace storage
{
...
#endif
    {
        ...
        // version 2
        std::list<boost::uint32_t> traffic_list_;    // 上传流量统计队列
        boost::uint32_t last_push_time_;             // 上次队列push时间
        // version 3
        ...
        boost::uint32_t data_rate_;                  // 文件码流率Bps
        // version 4
        string file_name_;                  // 文件名
        // version 5
        boost::uint32_t is_open_service_;            // 是否为开放服务
        // version 6
        ...
        bool Parse(const base::AppBuffer buf, boost::uint32_t version);
