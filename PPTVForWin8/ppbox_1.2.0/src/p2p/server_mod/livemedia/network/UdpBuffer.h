...
#ifndef _NETWORK_UDP_BUFFER_
#define _NETWORK_UDP_BUFFER_

#include "base/SubPieceBuffer.h"
...
//        , public framework::memory::MemoryPoolObjectNoThrow<UdpBuffer>
    ...
    public:
        UdpBuffer()
            : sub_piece_buffer_(new base::SubPieceContent)
        {
    ...
    public:
        ...
        void swap_sub_piece_buffer(
            base::SubPieceContent::pointer & buf)
        {
            assert(buf);
        ...
        {
            return (sub_piece_buffer_ && *sub_piece_buffer_) ? unspecified_bool_true : 0;
        }
    ...
    protected:
        ...
        {
			return base::buffer_static_cast<boost::uint8_t *>(*sub_piece_buffer_);
        }

        boost::uint8_t const * sub_piece_buffer() const
        {
			return base::buffer_static_cast<boost::uint8_t const *>(*sub_piece_buffer_);
        }

        size_t sub_piece_size() const
        {
            return base::SubPieceContent::sub_piece_size;
        }
    ...
    private:
        static const size_t head_size =  4+1+4+4+16+16+4+2;
        static const size_t tail_size = 1024;

    private:
        boost::asio::ip::udp::endpoint endpoint_;
        boost::uint8_t head_buffer_[head_size];
        base::SubPieceContent::pointer sub_piece_buffer_;
        boost::uint8_t tail_buffer_[tail_size];
    };

}

#endif // _NETWORK_UDP_BUFFER_
