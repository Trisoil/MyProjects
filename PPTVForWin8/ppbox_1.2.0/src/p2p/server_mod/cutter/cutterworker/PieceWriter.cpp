...
void on_fetch(boost::system::error_code const & ec, boost::shared_ptr<util::protocol::HttpClient> client_ptr)
{
	if (ec)
	{
		get_logger().printf(lilogger::kLevelDebug, "Report stat: [%d] %s", ec.value(), ec.message().c_str());
		get_logger().printf(lilogger::kLevelDebug, "Http stat: [%d] %s", client_ptr->get_response_head().err_code, client_ptr->get_response_head().err_msg.c_str());
	}
	client_ptr.reset();
}
void CPieceWriter::ReportReaderStatus( bool status )
{
	...
	std::string status_report_url = "http://live-cms.synacast.com/api_stream_status?GUID=" + guid_str_;
	if (status)
		status_report_url += "&Status=1";
	else
		status_report_url += "&Status=2";

	reporter_ptr->async_fetch_get(status_report_url, boost::bind(&on_fetch, boost::system::error_code(), reporter_ptr));
...
//						os << framework::container::make_array((unsigned char*)&piece_data_header_, PIECE_DATA_HEADER_T_SIZE);
				...
				get_logger().printf(lilogger::kLevelDebug, "Written [%d.piece.head], size [%d]", current_piece_id_, piece_header_.PIECE_LENGTH);
...
//							os << framework::container::make_array(&(checkpoint_tag_->buf_), checkpoint_tag_->size_);
				...
				get_logger().printf(lilogger::kLevelDebug, "Written [%d.piece], size [%d]", current_piece_id_, piece_header_.PIECE_LENGTH);
		...
		case STAT_NORMAL:
			{
				...
				get_logger().printf(lilogger::kLevelDebug, "Written [%d.flv], size [%d]", current_piece_id_, piece_length);
...
// 							file_path_.replace_extension("");
...
#endif // WIN32							error_value = e.code().value();
						}
				...
				get_logger().printf(lilogger::kLevelDebug, "Written [%d.block], size [%d]", current_piece_id_, piece_length);
