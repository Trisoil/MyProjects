...
FRAMEWORK_LOGGER_DECLARE_MODULE_LEVEL("MutexClient", 0);
...
namespace mutex
{
    namespace client
    {
        ...
        MutexClient::MutexClient(
            ...
            : io_svc_(io_svc)
            , keep_alive_timer_(new boost::asio::deadline_timer(io_svc))
        ...
        error_code MutexClient::start(void)
        {
            ...
            if (!ec) {
                ...
                LOG_S(Logger::kLevelInfor, "first select mutex server: " << current_addr.to_string());
                ...
                LOG_S(Logger::kLevelError, " ec: " << ec.message());
            }
            return ec;
        }
            ...
            error_code & ec)
        {
            ...
            keep_alive_timer_->expires_from_now(boost::posix_time::seconds(interval_));
            ...
            error_code const & ec)
        {
            ...
            if (not_alive_time_ >= 10 * 60) { // 10 minutes
                error_last_ = time_out;
            }
            if (!error_last_)
                send_keep_alive(error_last_);
            if (error_last_) {
                state_call_back_(error_last_);
                LOG_S(Logger::kLevelDebug2, "send_keep_alive: " << error_last_.message());
                cancel();
                return;
            }
            not_alive_time_ += interval_;
            keep_alive_timer_->expires_at(keep_alive_timer_->expires_at() + boost::posix_time::seconds(interval_));
            keep_alive_timer_->async_wait(boost::bind(&MutexClient::on_timer, this, _1));
        }
            ...
            error_code & ec)
        {
            if (try_left_ == 0) {
                ...
                if (!ec) {
                    ...
                    LOG_S(Logger::kLevelEvent, "change server: " << current_addr.host() << ":" << current_addr.port());
            ...
            if (!ec) {
                ...
                // LOG
                {
                    ...
                    LOG_S(Logger::kLevelDebug2, "current server: " << current_addr.host() << ":" << current_addr.port());
                    LOG_S(Logger::kLevelDebug2, "async_send_packet:" 
            ...
            std::streambuf & packet)
        {
            ...
            LOG_S(Logger::kLevelDebug2, "receive_callback:" 
            ...
            if (head.action == KeepaliveResponePackage::action) {
                ...
                if (keepalive_respone.passstate == 0) {
                    ...
                    error_last_ = (error::errors)(keepalive_respone.passstate + response_error_delta);
                    LOG_S(Logger::kLevelAlarm, "keepalive failed:" 
                        << " ec=" << error_last_.message());
                    state_call_back_(error_last_);
                    this->cancel();
                }
                ...
                if (packet.client_id.compare(client_id_) == 0 && 
                    ...
                    error_last_ = kickout;
                    LOG_S(Logger::kLevelAlarm, "kickout failed:" 
                        << " ec=" << error_last_.message());
                    state_call_back_(error_last_);
                    this->cancel();
                }
            } else {
                LOG_S(Logger::kLevelAlarm, "receive other action:" 
                    << head.action );
            }
        }
        ...
        void MutexClient::leave()
        {
            ...
            if (protocol_handle_) {
                LOG_S(Logger::kLevelAlarm, "send leave package in leave()");
