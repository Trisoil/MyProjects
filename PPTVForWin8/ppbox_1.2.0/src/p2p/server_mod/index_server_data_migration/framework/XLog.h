...
#   define LOG(level, type, message) do{\
        ...
        oss << message; \
        xframework::Log::Inst().WriteLog(level, type, oss.str()); } while(false)
#else
...
#		define LOGT(level, type, message) do{\
    ...
    oss << message; \
    xframework::Log::Inst().WriteLogT(level, type, oss.str()); } while(false)
#	else
#		define LOGT(level, type, message) do{\
    std::ostringstream oss; \
    oss << message; \
...
#   define RELEASE_LOG(message) do{\
        std::ostringstream oss; \
        oss << message; \
        xframework::Log::Inst().WriteLog(0, "", oss.str()); } while(false)
#endif
...
#   define RELEASE_OUTPUT(message) do{\
    std::ostringstream oss; \
    oss << message; \
    xframework::Log::Inst().GetDefaultLogWriter()->WriteLine(oss.str()); } while (false)
#endif

#define CONSOLE_LOG(message) do{\
    std::ostringstream oss; \
    oss << message; \
    xframework::Log::Inst().WriteConsoleLog(oss.str()); } while(false)

#define CONSOLE_OUTPUT(message) do{\
    std::ostringstream oss; \
    oss << message; \
    xframework::Log::Inst().WriteConsoleLine(oss.str()); } while(false)

#define STATISTIC_LOG(message) do{\
    std::ostringstream oss;\
    oss << message; \
    xframework::Log::Inst().GetStatisticLogWriter()->WriteLog(0, "", message); } while (false)
