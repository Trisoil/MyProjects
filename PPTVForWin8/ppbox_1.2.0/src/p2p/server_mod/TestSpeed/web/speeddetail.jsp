...
<%
	...
	String speedKey = SuperString.notNullTrim(request.getParameter("speedKey"));
	String s_resultId = SuperString.notNullTrim(request.getParameter("s_resultId"));
    ...
    String orderStr = order.length() == 0 ? " order by ResultValue desc " :
            " order by " + order;
    
    if(s_resultId.length()>0)whereStr.append(" and ResultID='"+s_resultId+"'");
    ...
    String sql="select * from speed_result where SpeedKey="+speedKey+whereStr+orderStr;
	...
	if(act.equalsIgnoreCase("addnote")){
		String sql_u="update speed_result set Note ='"+note+"' where ID ="+id;
...
<script type="text/javascript" language="javascript">
	function doSearch(){
		...
		var s_resultId=document.getElementById('s_resultId').value;
		//
		...
		searchForm.action='<%=curFile%>?act=search&speedKey='+<%=speedKey%>+'&s_resultId='+s_resultId+'&s_begindate_c='+s_begindate_c+'&s_enddate_c='+s_enddate_c;
		searchForm.submit();
	}		
	function doAddNote(id) {
		var url='speeddetail.jsp?act=addnote&id='+id+'&speedKey='+<%=speedKey%>;     
...
<table width="100%"  border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td class="title_text">结 果 列 表</td>
  </tr>
</table>
...
<table cellspacing="0" cellpadding="1" border="0" align="center" width="90%">
	...
	<tr>
      	<td nowrap="" width="70%">[<font color="#0066ff">查询</font>]
        	结果ID：<input type="text" value="<%=s_resultId%>" size="10" title="s_resultId" id="s_resultId" name="s_resultId">
...
<table width="100%"  border="0" cellspacing="1" cellpading="2">
  <tr>
    <td align="left" width="5%"><input name="Submit" type="button" class="BUTTON2" value=" 新 增 测 速 连 接 " onClick="location.href='speedmod.jsp'"></td>
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" class="table_bord">
  <tr class="table_bord_ttl">
    ...
    <td nowrap width="13%" onClick="title_click('<%=Util.getOrderUrl("ResultValue",order,request)%>')">结果值<%=Util.getOrderByImgHTML("ResultValue", order)%></td>
    <td nowrap width="12%">返回时间</td>
    <td nowrap width="13%">用户IP</td>
    <td nowrap width="15%">注释</td>
    <td nowrap width="15%">操作</td>
  </tr>
<%
    for(int i = 0; i < rs.size(); i++) {
        SpeedResult speed=rs.get(i).get(SpeedResult.class);
...
%>
   <tr class="<%=trclass%>" onMouseMove="cell_over(this);" onMouseOut="cell_out(this)" onClick="cell_click(this);">
    ...
    <td nowrap class="font22"><%=speed.getSpeedKey()%></td>
    <td nowrap class="font22"><%=speed.getResultID() %>--<%=serverMap.get(speed.getResultID()) %></td>
...
<%if(rs.size()==0){ %>
  <tr bgcolor="#FFFFFF" class="tr_class1">
    <td height="50" colspan="7"><div align="center">没有数据</div></td>
  </tr>
<%} %>
</table>
