...
void on_fetch(boost::system::error_code const & ec, boost::shared_ptr<util::protocol::HttpClient> client_ptr)
{
	if (ec)
	{
		printf("Report stat: [%d] %s\n", ec.value(), ec.message().c_str());
		printf("Http stat: [%d] %s\n", client_ptr->get_response_head().err_code, client_ptr->get_response_head().err_msg.c_str());
	}
	client_ptr.reset();
}
				...
				do {
					do {
						...
						if (NeedCheckPointTag() && checkpoint_tag_)
						{
							...
							double* check_id = (double*)(checkpoint_tag_->buf_.get() + CheckPointOffset);
							*check_id = (double)current_piece_id_;
							(*(boost::uint64_t*)check_id) = framework::system::BytesOrder::host_to_big_endian_longlong(*(boost::uint64_t*)check_id);
...
//						os << framework::container::make_array((unsigned char*)&piece_data_header_, PIECE_DATA_HEADER_T_SIZE);
				...
				printf("Written [%d.piece.head], size [%d]\n", current_piece_id_, piece_header_.PIECE_LENGTH);
		...
		case STAT_NORMAL:
			{
				...
				printf("Written [%d.flv], size [%d]\n", current_piece_id_, piece_length);
