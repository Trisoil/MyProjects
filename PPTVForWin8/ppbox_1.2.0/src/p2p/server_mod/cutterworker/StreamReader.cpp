...
, work_(ios_)
{
    ...
    {
        writer_ = new CPPLBlockWriter(path, piece_scale, piece_life_span);
        writer_->SetCheckpointScale(piece_scale);
				...
				case READING_HEAD:
					{
						...
						{
//							bool has_video = (flvHeader->flags & 0x05) != 4;
							FLVTagHeader_t* flvTagHeader = (FLVTagHeader_t*)(reading_buffer_ptr_->buf_.get() + FLV_FILEHEADER_T_SIZE);
				...
				case READING_FLV_TAGBODY:
					{
						...
						else if (is_header_over_)
						{
							assert(piece_ptr_->size() > 2);
							piece_ptr tmp_piece_ptr(new std::vector<liter_buffer_ptr>());

							tmp_piece_ptr->push_back((*piece_ptr_)[piece_ptr_->size() - 2]);
							tmp_piece_ptr->push_back((*piece_ptr_)[piece_ptr_->size() - 1]);
							piece_ptr_->pop_back();
							piece_ptr_->pop_back();
				...
				case READING_FLV_TAGHEAD:
					{
						...
						tag_ts = FLVTagHeader_t::get_tag_ts(flvTagHeader);
						if (got_first_data_tag_ == false 
							&& (cur_flv_tag_type_ == FLV_AUDIODATA || cur_flv_tag_type_ == FLV_VIDEODATA))
						{
							if (tag_ts < flv_tag_timestamp_)
							{
								flv_tag_timestamp_ += 500;
						...
						else
						{
							...
							if (tag_ts - flv_tag_timestamp_ > 100)
								printf("flv_ts_delta: === %d === \n", tag_ts - flv_tag_timestamp_);
							flv_tag_timestamp_ = tag_ts;
						}
						cur_flv_tag_ts_ = flv_tag_timestamp_ / cut_scale_;
...
//								should_cut_count_ += ts_delta;
						...
						else if (cur_flv_tag_ts_ < cur_piece_ts_)
						{
...
// 								ts_delta = (int)(cur_tag_ts - cur_piece_ts_ * cut_scale_) / cut_scale_;
							...
							else if (cur_flv_tag_ts_ != 0 && (int)(cur_piece_ts_ - cur_flv_tag_ts_) > 2)
							{
//								assert(0);
								// Restart
				...
				case READING_FLV_TAGBODY:
					{
						...
						if (cur_flv_tag_type_ == FLV_VIDEODATA)
						{
							VideoTagDataHeader_t* vHeader = (VideoTagDataHeader_t*)reading_buffer_ptr_->buf_.get();
							if ((vHeader->frameandcodetype & 0xf0) == 0x10)
							{
								if (!cut_with_kf_)
								{
...
//										cur_piece_ts_ = cur_flv_tag_ts_;
										printf("Cut point: < %d >, should cut count[ %d ]\n", cur_flv_tag_ts_, should_cut_count_);
//										can_cut_ = false;
/*
