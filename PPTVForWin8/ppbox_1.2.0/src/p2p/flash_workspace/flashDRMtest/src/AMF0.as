	...
	public class AMF0 {
		...
		public function readData(ba:ByteArray, type:int = -1):* {
			...
			switch(type) {
				...
				case REFERENCE_TYPE : return getObjectReference(ba.readUnsignedShort());
		...
		protected function getObjectReference(ref:int):Object {
			if (ref >= readObjectCache.length) {
				throw Error("AMF0::getObjectReference - Error : Undefined object reference '" + ref + "'");
				return null;
			}
		...
		protected function setObjectReference(ba:ByteArray, o:*):Boolean {
			...
			if (writeObjectCache != null && (refNum = hasItem(writeObjectCache, o)) != -1) {
				...
				if (writeObjectCache.length < MAX_STORED_OBJECTS) writeObjectCache.push(o);
				return true;
			}
		}
