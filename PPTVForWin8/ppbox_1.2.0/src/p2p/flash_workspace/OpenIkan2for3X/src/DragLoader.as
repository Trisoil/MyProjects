package
{
	...
	public class DragLoader extends EventDispatcher
	{
		...
		public function load():void
		{
			...
			dragLoader.load(dragHost + fileName);
		...
		private function onDragComplete(event:Event):void
		{
			...
			try
			{
				...
				for (var i:uint = 0; i < segmentList.length(); ++i)
				{
					...
					logger.info("segment " + i + " duration:" + duration + ", rid:" + rid + ", fileLength:" + fileLength);
					segments.push(new SegmentInfo(rid, duration, fileLength));
				}
