...
//Add for ppbox
void
AP4_StcoAtom::ReSetEntryCount(AP4_UI32 entry_count)
{
    AP4_UI32 * entrys = new AP4_UI32[entry_count];
    for (AP4_Ordinal i=0; i<m_EntryCount; i++) {
        entrys[i] = m_Entries[i];
    }
    m_EntryCount = entry_count;
    ...
    // reset size
    SetSize32(entry_count * 4 + 16);
}

AP4_Result
AP4_StcoAtom::AdjustChunkOffsets(AP4_Cardinal form, int delta)
{
    for (AP4_Ordinal i=form; i<m_EntryCount; i++) {
        m_Entries[i] += delta;
    }
