...
// Add
AP4_Result
AP4_StscAtom::GetChunkSampleIndexs(AP4_Ordinal   chunk_index,
                                        AP4_Ordinal & begin_sample_index,
                                        AP4_Ordinal & end_sample_index)
{
    AP4_Ordinal   samples = 0;
    for (AP4_Ordinal group = 0; group < m_Entries.ItemCount(); ++group) {
        AP4_Ordinal begin_chunk;
        if (0 == m_Entries[group].m_ChunkCount) {
            begin_chunk = chunk_index - m_Entries[group].m_FirstChunk + 1;
            begin_sample_index = m_Entries[group].m_FirstSample + 
                ((begin_chunk - 1) * m_Entries[group].m_SamplesPerChunk);
            end_sample_index   = m_Entries[group].m_FirstSample + 
                (begin_chunk * m_Entries[group].m_SamplesPerChunk) - 1;
            return AP4_SUCCESS;
        } else {
            if ((m_Entries[group].m_FirstChunk + m_Entries[group].m_ChunkCount) > chunk_index) {
                begin_chunk = chunk_index - m_Entries[group].m_FirstChunk + 1;
                begin_sample_index = m_Entries[group].m_FirstSample + 
                    ((begin_chunk - 1) * m_Entries[group].m_SamplesPerChunk);
                end_sample_index   = m_Entries[group].m_FirstSample + 
...
// Add for ppbox
AP4_Array<AP4_StscTableEntry> const &
AP4_StscAtom::GetEntries(void)
{
    return m_Entries;
}

void
AP4_StscAtom::SetEntry(AP4_Ordinal index, AP4_StscTableEntry stsc)
{
    m_Entries[index] = stsc;
}

//void
//AP4_StscAtom::SetLastItemChunkCount(AP4_Ordinal chunk_count)
//{
//    if (m_Entries.ItemCount() > 0) {
//        m_Entries[m_Entries.ItemCount() - 1].m_ChunkCount = chunk_count;
//    }
//}
