...
extern "C" {
    ...
    //打开一个视频
    PPBOX_DECL EXPORT_FUNC(PP_int32, PPBOX_Open)(
        PP_char const * playlink);

    PPBOX_DECL EXPORT_FUNC(PP_int32, PPBOX_OpenEx)(
        PP_char const * playlink, 
        PP_char const * format);

    typedef void (__cdecl *PPBOX_Open_Callback)(PP_int32);

    PPBOX_DECL EXPORT_FUNC(void, PPBOX_AsyncOpen)(
        PP_char const * playlink, 
    ...
    //暂停
    PPBOX_DECL EXPORT_FUNC(PP_int32, PPBOX_Pause)(void);

    //强制结束
    PPBOX_DECL EXPORT_FUNC(void, PPBOX_Close)(void);

    //获得有多少流
    PPBOX_DECL EXPORT_FUNC(PP_uint32, PPBOX_GetStreamCount)(void);
    ...
    //获得流的详细信息
    PPBOX_DECL EXPORT_FUNC(PP_int32, PPBOX_GetStreamInfo)(
        PP_uint32 index, 
    ...
    //获得流的详细扩展信息
    PPBOX_DECL EXPORT_FUNC(PP_int32, PPBOX_GetStreamInfoEx)(
        PP_uint32 index,
        PPBOX_StreamInfoEx *info);

    //获得总时长
    PPBOX_DECL EXPORT_FUNC(PP_uint32, PPBOX_GetDuration)(void);

    //获得播放长宽，像素
    PPBOX_DECL EXPORT_FUNC(PP_int32, PPBOX_GetWidthHeight)(
        PP_uint32 * pwidth, 
        PP_uint32 * pheight);

    //跳到某个时刻开始播放
    PPBOX_DECL EXPORT_FUNC(PP_int32, PPBOX_Seek)(
        PP_uint32 start_time);

    //获得AVC音频编码的AVCDecoderConfigurationRecord参数
    PPBOX_DECL EXPORT_FUNC(PP_int32, PPBOX_GetAvcConfig)(
    ...
    //同步读取Sample接口，不阻塞
    PPBOX_DECL EXPORT_FUNC(PP_int32, PPBOX_ReadSample)(
        PPBOX_Sample * sample);

    struct PPBOX_SampleEx
    {
        PP_uint32 stream_index;     // 流的编号
        PP_uint32 start_time;       // Sample对应的时间戳, 单位是毫秒
    ...
    PPBOX_DECL EXPORT_FUNC(PP_int32, PPBOX_ReadSampleEx)(
    ...
    PPBOX_DECL EXPORT_FUNC(PP_int32, PPBOX_ReadSampleEx2)(
    ...
    // length: 预设的下载内存缓冲的大小
    PPBOX_DECL EXPORT_FUNC(void, PPBOX_SetDownloadBufferSize)(
        PP_uint32 length);

    PPBOX_DECL EXPORT_FUNC(void, PPBOX_SetHttpProxy)(
        PP_char const * addr);

    PPBOX_DECL EXPORT_FUNC(void, PPBOX_SetDownloadMaxSpeed)(
        PP_uint32 speed);
    ...
    PPBOX_DECL EXPORT_FUNC(void, PPBOX_SetPlayBufferTime)(
        PP_uint32 time);
    ...
    //    ppbox_success      表示成功
    //    其他数值表示失败
    PPBOX_DECL EXPORT_FUNC(PP_int32, PPBOX_GetPlayMsg)(
    ...
    //    ppbox_success      表示成功
    //    其他数值表示失败
    PPBOX_DECL EXPORT_FUNC(PP_int32, PPBOX_GetDownMsg)(
    ...
    //    ppbox_success      表示成功
    //    其他数值表示失败
    PPBOX_DECL EXPORT_FUNC(PP_int32, PPBOX_GetDownSedMsg)(
